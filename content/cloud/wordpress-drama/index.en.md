...
How to solve this problem? Open source communities have actually provided their own answer. In "[Redis Going Closed Source is a Disgrace to 'Open-Source' and Public Clouds](/en/db/redis-oss/)", I've already analyzed in detail — for example, in the database field, it's not hard to see that relationships between leading database companies/communities and cloud vendors have been escalating in recent years. [Redis changed its open source license to RSAL/SSPL](/en/db/redis-oss/), [ElasticSearch changed to AGPLv3](https://www.elastic.co/blog/elasticsearch-is-open-source-again), [MongoDB uses the more restrictive SSPL](https://www.mongodb.com/legal/licensing/server-side-public-license), MinIO and Grafana also switched to the more restrictive AGPLv3. [Oracle slacks off on MySQL open source version](/en/db/mysql-vs-pgsql/), and even [the most friendly PostgreSQL ecosystem is starting to show different voices](/pg/pg-license/).
