...
12 月 11 日，OpenAI 出现了全球范围内的不可用故障，影响了 ChatGPT，API，Sora，Playground 和 Labs 等服务。影响范围从 12 月 11 日下午 3:16 至晚上 7:38 期间，持续时间超过四个小时，产生显著影响。
...
其实这个故障和去年双十一  [阿里云全球史诗故障](/cloud/aliyun/)  非常类似。都是全球控制面不可用，根因都是循环依赖（以及测试/发布灰度不足）。无非是阿里云是 OSS 和 IAM 之间循环依赖，OpenAI 是 DNS 和 K8S 的循环依赖。

---

循环依赖是架构设计中的大忌，就像在基础设施中放了炸药包，容易被一些临时性偶发性故障点爆。这次故障再次为我们敲响警钟。当然这次故障的原因还可以进一步深挖，比如测试/灰度不足，以及 架构杂耍：

比如 K8S 官方建议的 [最大集群规模是 5000 节点](https://kubernetes.io/docs/setup/best-practices/cluster-large/)，而我还清晰记得 OpenAI 发表过一篇吹牛文章：《[我们是如何通过移除一个组件来让 K8S 跑到 7500 节点的](https://newsletter.betterstack.com/p/how-openai-scaled-kubernetes-to-7500)》
—— 不仅不留冗余，还要超载压榨 50%，最终，这次还真就在集群规模上翻了大车。
...
[无双删库：Google 云爆破了大基金的整个云账户](/cloud/gcp-unisuper/)
...
发布版本：[微信公众号](https://mp.weixin.qq.com/s/ze4y9x7VpPmibitH7tAGMQ)
